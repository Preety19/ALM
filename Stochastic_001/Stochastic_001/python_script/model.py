...
def simulations(df):
    ...
    num_simulations = 1001
    # Generate random shocks for each maturity
    shocks = np.random.normal(0, 0.03, (num_simulations, len(df)))
    # Perform Monte Carlo simulation
    simulated_yield_curves = df.copy()
    for i in range(1, num_simulations):
        simulated_yield_curves[f'Simulation_{i}'] = df['Day_1'] + shocks[i]
    sim_curves = simulated_yield_curves.copy(deep=True)
    sim_curves.drop(['Maturity', 'Day_1', 'Day_2'], axis = 1, inplace = True)    
    MC = plt.plot(sim_curves)
    Actual=plt.plot(simulated_yield_curves['Day_1'],color='black',label='Actual Yield Curve')
    plt.legend(loc='best')
...
def final_total_difference(df, sim_curves,assets_df, liabilities_df, multiplier_dict):
    ...
    for i in range (0, len(sim_curves.columns)):
        ...
        df2['Mid_point'] = mids
        zcyc = []
        a = zcyc.append(round(sim_curves.iloc[:,i][0]+((sim_curves.iloc[:,i][1]-sim_curves.iloc[:,i][0]) * 
        ...
        a = zcyc.append(round(sim_curves.iloc[:,i][1]+((sim_curves.iloc[:,i][2]-sim_curves.iloc[:,i][1]) * 
        ...
        a = zcyc.append(round(sim_curves.iloc[:,i][2]+((sim_curves.iloc[:,i][3]-sim_curves.iloc[:,i][2]) * 
            (Mid_point['Over 1 year and upto 1_5 year'] - df['Maturity'][2])/(df['Maturity'][3] - df['Maturity'][2])),2))

        a = zcyc.append(round(sim_curves.iloc[:,i][3]+((sim_curves.iloc[:,i][4]-sim_curves.iloc[:,i][3]) * 
        ...
        for j in mid_values:
            a = sim_curves.loc[df['Maturity'] == j, f'Simulation_{i+1}'].values[0] if 0 in df['Maturity'].values else None
            zcyc.append(a)
        zcyc.extend([0,0,0])
        df2['ZCYC'] = zcyc 
        ...
        updated_a = normal_scenario_sensitive_total - a
        total_pv_difference_parallel_up.append(updated_a)
        ...
        updated_a = normal_scenario_sensitive_total - a        
        total_pv_difference_parallel_down.append(updated_a)
        ...
        updated_a = normal_scenario_sensitive_total - a        
        total_pv_difference_Short_rate_Shock_up.append(updated_a)
        ...
        updated_a = normal_scenario_sensitive_total - a        
        total_pv_difference_Short_rate_Shock_down.append(updated_a)
        ...
        updated_a = normal_scenario_sensitive_total - a                
        total_pv_difference_steepener.append(updated_a)
        ...
        updated_a = normal_scenario_sensitive_total - a                
        total_pv_difference_flattener.append(updated_a)
...
if __name__ == '__main__': 
    ...
    df = data_load(path)
    sim_curves = simulations(df)
    assets_df, liabilities_df, total_df2, nii_values_list, assets_df_nii, liabilities_df_nii,total_df2_nii = total_gap(path)
    
    maximum_value_parallel_up, maximum_value_parallel_down, maximum_value_Short_rate_Shock_up, maximum_value_Short_rate_Shock_down, maximum_value_steepener, maximum_value_flattener = final_total_difference(df, sim_curves,assets_df, liabilities_df, multiplier_dict)
    ...
    final_df = df = pd.DataFrame({'Period': shocks,'GAP': max_loss_values,'NII': nii_values_list})
